...
```{r}
#Figure S2
#First create a dataframe for adult big brown bat capture counts by time-step, sex, and site
EPFU_count_adult<- count(EPFU_adult, vars = c('sex', 'site_mask', 'disease_time_step')) %>% 
  group_by(sex)

#Create a vector of number of capture / number of sites for each time-step and sex
cap_per_site<- c(sum(EPFU_count_adult$freq[EPFU_count_adult$disease_time_step == 'pre-invasion' & EPFU_count_adult$sex == 'female'])/length(unique(EPFU_count_adult$site_mask[EPFU_count_adult$disease_time_step == 'pre-invasion' & EPFU_count_adult$sex == 'female'])),
  
           sum(EPFU_count_adult$freq[EPFU_count_adult$disease_time_step == 'invasion' & EPFU_count_adult$sex == 'female'])/length(unique(EPFU_count_adult$site_mask[EPFU_count_adult$disease_time_step == 'invasion' & EPFU_count_adult$sex == 'female'])), 
  
          sum(EPFU_count_adult$freq[EPFU_count_adult$disease_time_step == 'epidemic' & EPFU_count_adult$sex == 'female'])/length(unique(EPFU_count_adult$site_mask[EPFU_count_adult$disease_time_step == 'epidemic' & EPFU_count_adult$sex == 'female'])),
  
          sum(EPFU_count_adult$freq[EPFU_count_adult$disease_time_step == 'established' & EPFU_count_adult$sex == 'female'])/length(unique(EPFU_count_adult$site_mask[EPFU_count_adult$disease_time_step == 'established' & EPFU_count_adult$sex == 'female'])),
  
          sum(EPFU_count_adult$freq[EPFU_count_adult$disease_time_step == 'pre-invasion' & EPFU_count_adult$sex == 'male'])/length(unique(EPFU_count_adult$site_mask[EPFU_count_adult$disease_time_step == 'pre-invasion' & EPFU_count_adult$sex == 'male'])),
  
          sum(EPFU_count_adult$freq[EPFU_count_adult$disease_time_step == 'invasion' & EPFU_count_adult$sex == 'male'])/length(unique(EPFU_count_adult$site_mask[EPFU_count_adult$disease_time_step == 'invasion' & EPFU_count_adult$sex == 'male'])), 
  
          sum(EPFU_count_adult$freq[EPFU_count_adult$disease_time_step == 'epidemic' & EPFU_count_adult$sex == 'male'])/length(unique(EPFU_count_adult$site_mask[EPFU_count_adult$disease_time_step == 'epidemic' & EPFU_count_adult$sex == 'male'])),
  
          sum(EPFU_count_adult$freq[EPFU_count_adult$disease_time_step == 'established' & EPFU_count_adult$sex == 'male'])/length(unique(EPFU_count_adult$site_mask[EPFU_count_adult$disease_time_step == 'established' & EPFU_count_adult$sex == 'male'])))

#then create a vector for age
sex2<- c(rep('female', 4), rep('male', 4))
...
disease_sex_cps<- data.frame(cap_per_site, sex2, disease_time_step2)

#fix levels in the new dataframe
disease_sex_cps$sex2<- factor(disease_sex_cps$sex2, levels = c('female', 'male'))

disease_sex_cps$disease_time_step2<- factor(disease_sex_cps$disease_time_step2, levels = c('pre-invasion', 'invasion', 'epidemic', 'established'))

#captures per site by disease time-steps and sex
FigS2<- ggplot(disease_sex_cps, aes(sex2, cap_per_site, color = disease_time_step2, fill = disease_time_step2))+
  geom_bar(stat="identity")+
  scale_color_manual(values= c('#83CAFF', '#FF950E', '#FFD320', '#579D1C')) +
  scale_fill_manual(values= c('#83CAFF', '#FF950E', '#FFD320', '#579D1C')) +
  theme_bw()+
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
        panel.background = element_rect(fill = "transparent", colour = NA),
...
#Figure S3
figS3a<- ggplot(data = fem_EPFU, aes(x= county_centroid_lat, y= mass, color = disease_time_step))+
...
figS3b<- ggplot(data = male_EPFU, aes(x= county_centroid_lat, y= mass, color = disease_time_step))+
...
#Figure S4
figS4<- ggplot(fem_EPFU, aes(lat_category, mass, color = disease_time_step, fill = disease_time_step))+
  facet_wrap(~repstat) + 
...
figS5<- ggplot(data = ems_fem_mass_var_mod2_df, aes(x = lat_category, y = emmean, color= lat_category)) +
...
figS6<- ggplot(data = ems_fem_mass_var_mod2_df, aes(x = disease_time_step, y = emmean, color= disease_time_step)) +
...
#Figure S7
figS7<- ggplot(male_EPFU, aes(lat_category, mass, color = disease_time_step, fill = disease_time_step))+
  geom_point(alpha = 0.05, position = position_jitter(width = 0.3)) +
  geom_boxplot(width = 0.45, alpha = 0.4, position = position_dodge(.7), lwd = 1)+
